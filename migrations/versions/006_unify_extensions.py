...
def upgrade() -> None:
    ...
    op.add_column("extensions", sa.Column("type", sa.String(20), nullable=False, server_default="phone"))
    ...
    conn = op.get_bind()
    agents = conn.execute(sa.text("SELECT id, extension_number, name, enabled FROM ai_agents")).fetchall()
    for agent in agents:
        # Check if extension already exists with this number
        existing = conn.execute(
            sa.text("SELECT id FROM extensions WHERE number = :num"),
            {"num": agent[1]}
        ).fetchone()
        if not existing:
            conn.execute(
